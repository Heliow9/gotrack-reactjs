...
export default function ComandaScreen({ navigation, route }) {
  ...
  const fetchMpStatusFromApi = async (rid) => {
    ...
    try {
      ...
      const doc = res?.data?.restaurante || res?.data || {};
      const conectado = doc?.mercadoPago?.conectado === true;
  ...
  useEffect(() => {
    (async () => {
      ...
      const conectadoSessao = !!s?.restaurante?.mercadoPago?.conectado;
  ...
  const abrirPix = async () => {
    ...
    if (!mpConectado) {
      Alert.alert("Pix indisponível", "Restaurante não conectado ao Mercado Pago (OAuth).");
      return;
    }
        ...
        <View style={styles.modalOverlay}>
          <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={{ width: "100%" }}>
            <View style={[styles.modalCard, { width: "100%", maxWidth: 520 }]}>
              ...
              <Pressable
                onPress={abrirPix}
                disabled={!mpConectado || pixLoading}
                style={({ pressed }) => [
                  styles.payOption,
                  pressed && mpConectado && { opacity: 0.92 },
                  (!mpConectado || pixLoading) && { opacity: 0.45 },
                ]}
              >
                <View style={styles.payIcon}>
                ...
                <View style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <Text style={styles.paySub} numberOfLines={2}>
                    {mpConectado ? "QR Code / Copia e cola" : "Indisponível (Mercado Pago não conectado)"}
                  </Text>
                </View>
