...
const RESUMO_CACHE_KEY = "garcom:dashboard:resumo";
...
const normalizeResumo = (data = {}, session = null) => {
  ...
  const vendas = Number(data?.vendasHojeGarcom ?? data?.vendasHoje ?? data?.vendasTurno ?? data?.totalTurno ?? 0) || 0;
  ...
  const ranking = Array.isArray(data?.rankingGarcons) && data.rankingGarcons.length
    ? data.rankingGarcons
...
export default function HomeScreen({ navigation, onLogout }) {
  ...
  const fetchDashboard = useCallback(async ({ silent = false } = {}) => {
    try {
      ...
      setDashboard(normalizeResumo(result.data, s));
    ...
    <View style={styles.root}>
      ...
      <ScrollView style={styles.content} contentContainerStyle={styles.contentInner} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} />}>
        ...
        <View style={styles.kpiGrid}>
          ...
          <Kpi icon="cash-outline" label="Vendas do turno" value={moneyBRL(dashboard.vendasTurno)} sub="Seu login hoje" />
