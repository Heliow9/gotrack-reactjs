...
const PEDIDOS_CACHE_KEY = "garcom:pedidos:list";
...
const pickNumero = (p) => p?.numeroPedido || p?.numero || p?.codigo || "—";
...
export default function PedidosScreen({ navigation }) {
  ...
  const fetchPedidos = useCallback(async () => {
    try {
      const result = await cachedApiGet({
        key: PEDIDOS_CACHE_KEY,
        request: () => api.get("/api/garcons/app/pedidos", { params: { limit: 200 } }),
        fallback: [],
      });
  ...
  }, [animateUpdate, normalizeResponse]);
  ...
  const stats = useMemo(() => {
    ...
    const producao = pedidos.filter((p) => normalizeStatus(p?.status) === "em_producao").length;
    const totalHoje = pedidos.reduce((acc, p) => acc + Number(pickTotal(p) || 0), 0);
    return { ativos, producao, totalHoje };
  }, [pedidos]);
  ...
  const pedidosFiltrados = useMemo(() => {
    ...
    return pedidos
      ...
      .filter((p) => {
        if (!q) return true;
        const hay = [pickNumero(p), pickCliente(p), p?.telefoneCliente, p?.mesaNumero, p?.origem, p?.formaPagamento]
          .map(safeText)
          .join(" ")
          .toLowerCase();
    ...
    <View style={styles.root}>
      <LinearGradient colors={headerGradient} style={styles.header}>
        ...
        <View style={styles.kpiRow}>
          ...
          <View style={styles.kpiCard}><Text style={styles.kpiValue}>{money(stats.totalHoje)}</Text><Text style={styles.kpiLabel}>total listado</Text></View>
        </View>
      </LinearGradient>
      ...
      >
        ...
        <View style={styles.searchCard}>
          ...
          <Text style={styles.updatedText}>{lastUpdatedAt ? `Atualizado ${fmtDate(lastUpdatedAt)}${fromCache ? " • cache" : ""}` : "Sincronizando..."}</Text>
        </View>
