...
function ConfigProdutoModal({ visible, produto, onClose, onConfirm }) {
  ...
  const exigeSaboresExatos = isPizza && arr(produto?.saboresDisponiveis).length > 1 && maxSabores > 1;
  ...
  const confirmar = () => {
    if (!produto) return;
    if (isPizza && arr(produto.saboresDisponiveis).length) {
      if (exigeSaboresExatos && saboresSelecionados.length !== maxSabores) return setErro(`Escolha exatamente ${maxSabores} sabor(es).`);
      if (!exigeSaboresExatos && saboresSelecionados.length === 0) return setErro("Escolha pelo menos um sabor.");
    }
